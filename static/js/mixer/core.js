...
class StemMixer {
    ...
    async loadStems() {
        try {
            this.log('Chargement des stems...');
            
            // Vérifier si nous avons des informations d'extraction avec les chemins de sortie
            let stemFiles = [];
            
            if (window.EXTRACTION_INFO && window.EXTRACTION_INFO.output_paths) {
                this.log('Utilisation des chemins de stems depuis EXTRACTION_INFO');
                // Utiliser les chemins de stems fournis
                ...
                // Fallback: essayer les stems standards
                this.log('Fallback: essayer les stems standards...');
                const standardStems = ['vocals', 'drums', 'bass', 'guitar', 'piano', 'other'];
                stemFiles = standardStems.map(stem => ({
                    name: stem,
                    url: `/api/extracted_stems/${this.encodedExtractionId}/${stem}`
                }));
            }
            
            this.log(`Tentative de chargement de ${stemFiles.length} stems`);
            ...
            // Compter les stems chargés avec succès
            let successCount = 0;
            results.forEach((result, index) => {
                if (result.status === 'fulfilled') {
                    successCount++;
                    this.log(`Stem ${stemFiles[index].name} chargé avec succès`);
                } else {
                    this.log(`Erreur lors du chargement du stem ${stemFiles[index].name}: ${result.reason}`);
                }
            });
            
            if (successCount === 0) {
                throw new Error('Aucun stem n\'a pu être chargé');
            }
            
            this.log(`${successCount} stems chargés sur ${stemFiles.length}`);

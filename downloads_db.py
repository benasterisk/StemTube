...
def resolve_file_path(stored_path):
    ...
    to new locations by extracting the relative path and resolving it from the current app root.
    ...
    # Pattern to match old absolute paths
    # Matches: /opt/stemtube/StemTube-dev/core/downloads/...
    # or: /any/path/StemTube-dev/core/downloads/...
    old_path_pattern = r'^.*?/StemTube-dev/(core/downloads/.*)$'

    match = re.match(old_path_pattern, stored_path)
    if match:
        # Extract the relative path part (core/downloads/...)
        relative_path = match.group(1)
        # Resolve it from current app root
        resolved = APP_ROOT / relative_path
        return str(resolved)

    # If it's already a relative path starting with core/downloads
    if stored_path.startswith('core/downloads/'):
        resolved = APP_ROOT / stored_path
        return str(resolved)

...
StemTube supports two methods to get audio files into the system.

### Method 1: YouTube Download

**No API key required** - Uses aiotube + yt-dlp.
...
2. **Paste in StemTube**:
   - Go to main page (home icon)
   - Find "YouTube Download" section
   - Paste URL in text field
   - Click "Download"

3. **Monitor Progress**:
   - Real-time progress bar shows download %
   - Speed and ETA displayed
   - Downloads appear in "Your Downloads" list when complete

4. **Results**:
   - Audio file saved in `downloads/global/VIDEO_ID/`
   - Title and duration automatically detected
   - File deduplicated globally (downloaded once, accessible to all users with access)

**Supported Formats**:
- Best quality audio automatically selected
- Converted to consistent format (varies by source)
- Typical size: 3-8 MB per minute

**Troubleshooting**:
- "Video unavailable": Check if video is public and accessible in your region
- "Download failed": Video may be restricted or require authentication
- Slow download: Depends on YouTube servers and your internet connection

### Method 2: File Upload

**Upload your own audio files** (local files only).
